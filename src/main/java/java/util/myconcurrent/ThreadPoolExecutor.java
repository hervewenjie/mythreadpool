...
/**
 * Created by chengwenjie on 2018/12/25.
 */
...
//        handler.rejectedExecution(command, this);
    ...
    // add worker, start worker, run task
    private boolean addWorker(Runnable firstTask, boolean core) {
        ...
        try {
            ...
            if (t != null) {
                ...
                // if added, start thread
                if (workerAdded) {
                    // only thread started, not run worker's task
                    t.start();
                    workerStarted = true;
                }
            }
    ...
    private Runnable getTask() {
        boolean timedOut = false; // did last poll() timeout
    ...
    // repeatedly get tasks from queue and execute
    ...
    // main method
    // execute given task sometime in the FUTURE
    public void execute(Runnable command) {
        if (command == null) throw new NullPointerException();


        int c = ctl.get();
        // not yet core size, add and return
        ...
        // already core size, add to queue
        ...
        // add queue failed, add worker directly
